    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none flex justify-center items-center">
        {sparkles.map((sparkle) => (
          <CustomIconSparkle icon={icon} key={sparkle.id} {...sparkle} />
        ))}
      </div>

      {/* Render the button or any other content */}
      <div className="relative font z-10" onClick={addMoreIcons}> {/* Trigger icon generation on click */}
        {text}
      </div>
    </div>
  );
...
const CustomIconSparkle: React.FC<Sparkle> = ({ id, x, y, delay, scale, icon }) => {
    const maxScale = 0.5;
